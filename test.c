...
struct map {
	...
	char height, width; // Map properties
} board;
...
//	if (mapfile == NULL) {printw("Couldn't open map");} 
	...
	while (fgets(charbuffer, 3, mapfile)) {
		if (*charbuffer != '\n') {col++;}
		else {row++; width = col; col = 0;} // optimize
	}
	height = row;
...
/*** Start game ***/
void play() {
	struct position pos = {1, 1}; // Initial player position
	startpos.row = 0;
	startpos.col = 0; // Upper left window corner for printing
	char input, lastmove;
	board.map[pos.row][pos.col] = PLAYER_STRING;
	wrefresh(win.window);
	while (1) { 
		startpos = scrollwindow(startpos, pos, lastmove);
